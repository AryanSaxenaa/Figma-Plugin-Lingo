...
**Update:** A public CORS proxy is **no longer required**. The UI calls `https://engine.lingo.dev` directly (listed in `manifest.json` `networkAccess`). After `npm run build`, re-import the dev plugin in Figma so it loads the latest `dist/`.
